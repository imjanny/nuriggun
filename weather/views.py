...
def load_weather(nx, ny):
    ...
    for item in items['item']:
        ...
        # 하늘상태: 없음(0), 비(1), 비/눈(2), 눈(3), 빗방울(5), 빗방울눈날림(6), 눈날림(7)
...
def cron_weather():
    ...
    scheduler.add_job(delete_weather, 'cron', hour='15', id='cron_delete_weather')
    scheduler.add_job(save_weather, 'cron', hour='2,6,10,14,18,22', id='cron_save_weather')
    scheduler.start()
...
class WeatherView(APIView):
    ...
    def get(self, request):
        weather = WeatherData.objects.order_by('-timestamp')[:7]
